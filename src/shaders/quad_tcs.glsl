...
void main() {
    ...
    vec3 v01 = 2 * dot(p1 - p0, n0 + n0) / dot(p1 - p0, p1 - p0);
    vec3 h01 = n0 + n1 - v01 * (p1 - p0);

    vec3 v12 = 2 * dot(p2 - p1, n1 + n1) / dot(p2 - p1, p2 - p1);
    vec3 h12 = n1 + n2 - v12 * (p2 - p1);

    vec3 v23 = 2 * dot(p3 - p2, n2 + n2) / dot(p3 - p2, p3 - p2);
    vec3 h23 = n2 + n3 - v23 * (p3 - p2);

    vec3 v03 = 2 * dot(p3 - p0, n0 + n0) / dot(p3 - p0, p3 - p0);
